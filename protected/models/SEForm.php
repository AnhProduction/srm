...
* 
*/
class SEForm extends CFormModel
{
	public $MaSV;
	public $HoDem;
	public $Ten;
	...
	public function rules()
	{
		return array(
			...
			array('HoDem, Ten, HoTenBo, HoTenMe, TenChuTro, DanToc, TonGiao','match', 'pattern' => "/^[a-zA-Z_ÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯĂẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼỀỀỂưăạảấầẩẫậắằẳẵặẹẻẽềềểỄỆỈỊỌỎỐỒỔỖỘỚỜỞỠỢỤỦỨỪễệỉịọỏốồổỗộớờởỡợụủứừỬỮỰỲỴÝỶỸửữựỳỵỷỹ\s]+$/u", 'message'=>"<br/><div class=\"label label-danger\">{attribute} chứa ký tự không hợp lệ!</div>"),
			...
			//array('MaSV', 'checkMSV'),
			array('NgayVaoDang, NgayVaoDoan, NgayDK, NgayKT, NgayDKTro, NgheBo, NgheMe, DiaChiCuTru, SoThe, SoPhong, SoNha, DoiTuong','KeepValue'),
		);
	}
    ...
    {
        $All = HSSV::model()->findAll("SVID<>:SV AND MaSV=:MSV", array(':SV'=>$this->SVID,':MSV'=> $this->MaSV));
    	if(count($All)){
